...
class TipRegionContactForceLogger(Sofa.Core.Controller):
    ...
    def _closest_contact_pair(self):
        ...
        try:
            n = int(cl.getNumberOfContacts())
        except Exception:
            return None
        if n <= 0:
            return None

        # Distances usually exist even when points are hard to parse
        try:
        ...
        # Try to get points, but don't fail the whole function if layout differs
        try:
            cps = cl.getContactPoints()
            if cps is None or len(cps) <= i:
                return (dist, None, None, n)

            item = cps[i]

            # Common: (id1, p1, id2, p2)
            p1 = np.array(item[1], dtype=float)
            p2 = np.array(item[3], dtype=float)
            return (dist, p1, p2, n)
    ...
    def onAnimateEndEvent(self, event):
        ...
        if cc is not None:
            ...
            # nC is typically same as nC_total, but keep total as authoritative
